/*
 *  
 *  Using a specific function sliderDraw() to slider drawing.
 *
 */

#include <Adafruit_GFX.h>    // Core graphics library
#include <SPI.h>
#include <Wire.h>      // this is needed even tho we aren't using it
#include <Adafruit_ILI9341.h>
#include <Adafruit_STMPE610.h>

// This is calibration data for the raw touch data to the screen coordinates
#define TS_MINX 150
#define TS_MINY 130
#define TS_MAXX 3800
#define TS_MAXY 4000

// The STMPE610 uses hardware SPI on the shield, and #8
#define STMPE_CS 8
Adafruit_STMPE610 ts = Adafruit_STMPE610(STMPE_CS);

// The display also uses hardware SPI, plus #9 & #10
#define TFT_CS 10
#define TFT_DC 9
Adafruit_ILI9341 tft = Adafruit_ILI9341(TFT_CS, TFT_DC);


// Slider parameters
#define SLID_INIT_POSx 30
#define SLID_INIT_POSy 270
#define SLID_WIDTHx 180
#define SLID_HEIGHTy 15


void setup() {

  Serial.begin(9600);
  Serial.println(F("Touch Paint!"));
  
  tft.begin();
  if (!ts.begin()) {
    Serial.println("Couldn't start touchscreen controller");
    while (1);
  }
  
  Serial.println("Touchscreen started");
  tft.fillScreen(ILI9341_BLACK);
  tft.drawRect(SLID_INIT_POSx-1, SLID_INIT_POSy-1, SLID_WIDTHx+1, SLID_HEIGHTy+2, ILI9341_WHITE);
}


void loop() { 
  // Wait for a touch
  if (! ts.touched()) {
    return;
  }

  // Retrieve a point  
  TS_Point p = ts.getPoint(); 
  // Scale from ~0->4000 to tft.width using the calibration #'s
  p.x = map(p.x, TS_MINX, TS_MAXX, 0, tft.width());
  p.y = map(p.y, TS_MINY, TS_MAXY, 0, tft.height());

  sliderDraw(SLID_INIT_POSx, SLID_WIDTHx, SLID_INIT_POSy, SLID_HEIGHTy, p.x, p.y);

} // void loop() end

unsigned long sliderDraw(int slid_posx, int slid_w, int slid_posy, int slid_h, int ptx, int pty) {
  
    // Right limiter for slider
    int lim_posx = slid_posx + slid_w;
    int lim_posy = slid_posy;
    int lim_wid = tft.width() - slid_posx - slid_w;
    int lim_h = slid_h;

    int corr_wid = slid_posx + slid_w - ptx -1;

    int LIM_POSx = slid_posx + 75;
    int LIM_POSy = slid_posy - 20;
    int LIM_SIZ = 2;

    int pax=0;

    if((pty > (slid_posy-slid_h*2)) && (pty < (slid_posy + slid_h*2))) { 
    if ((ptx > slid_posx) && (ptx < (slid_posx + slid_w))) {


      // filling left slider side.
      tft.fillRect(slid_posx, slid_posy, ptx, slid_h, ILI9341_RED);
      // filling right slider side.
      tft.fillRect(ptx, slid_posy, corr_wid, slid_h, ILI9341_BLACK);

      // fixing the end.
      tft.fillRect(lim_posx, lim_posy, lim_wid, lim_h, ILI9341_BLACK);
      // fixing the border.
      tft.drawRect(slid_posx-1, slid_posy-1, slid_w+1, slid_h+2, ILI9341_WHITE);

      
      printText(LIM_POSx, LIM_POSy, pax, ILI9341_BLACK, LIM_SIZ);
      printText(LIM_POSx, LIM_POSy, ptx, ILI9341_WHITE, LIM_SIZ);
      pax=ptx;
    }
  }
  
    Serial.println("Inside function, after if");        // it is strange that I need this debug order to make it work.
}


unsigned long printText(int x, int y, int strg, int col, int sz) {
  unsigned long start = micros();
  //tft.setRotation(1);
  tft.setCursor(x, y);
  tft.setTextColor(col); tft.setTextSize(sz);
  tft.println(strg);
  return micros() - start;
}
